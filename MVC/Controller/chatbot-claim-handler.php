<?php
/**
 * Chat-assisted claim creation (Foovia / Wilson).
 *
 * Workflow (server):
 * 1. Accept POST JSON: { "description": "..." } (user text from chat after they agreed).
 * 2. Call Gemini (same model/key as chat-bot-config) with a classifier prompt; expect JSON { "type": "..." }.
 * 3. Map the model output to exactly one of: Authentication, Subscription, Other (same values as add_rec_page.php).
 * 4. Insert via Controller_reclamation::add_reclamation (same path as manual form).
 *
 * Response: { "success": true, "type": "...", "message": "..." } or { "error": "..." }.
 */
...
const CLAIM_TYPES = ['Authentication', 'Subscription', 'Other'];

$configPath = __DIR__ . '/../Model/chat-bot-config.php';
...
/**
 * Ask Gemini for a single JSON object: {"type":"Authentication"|"Subscription"|"Other"}.
 */
function claim_classify_description(string $description, string $geminiModel): string
{
    $system = 'You are a strict classifier for Foovia support claims. '
        ...
        . 'The value must be exactly one of these strings: Authentication, Subscription, Other. '
        ...
        . 'Other = anything that does not clearly fit the first two. '
