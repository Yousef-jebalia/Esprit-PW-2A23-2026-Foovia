...
foreach ($categoryRows as $categoryRow) {
  ...
  $categories[$categoryName] = $categoryController->count_recipes_by_category($categoryId);
    ...
    <script>
      ...
      (function() {
        ...
        const getMatchName = (match) => {
          ...
          return match.name || match.name_ing || match.detected || '';
        };
        ...
        const getMatchId = (match) => {
          ...
          return match.id || match.id_ing || '';
        };

        const renderAiResults = (matches, unmatched = []) => {
          clearAiResults();
          const seen = new Set();
          const renderChip = (name, isUnmatched) => {
            ...
            chip.className = isUnmatched
              ? 'ingredient-picture-ai-tag is-unmatched'
              : 'ingredient-picture-ai-tag';
            chip.textContent = normalized;
            if (isUnmatched) {
              chip.title = 'Not found in database';
            }
            aiResults.appendChild(chip);
          };

          (Array.isArray(matches) ? matches : []).forEach((match) => {
            renderChip(getMatchName(match), false);
          });
          (Array.isArray(unmatched) ? unmatched : []).forEach((name) => {
            renderChip(name, true);
          });
        };
        ...
        const applyDetectedMatches = (matches) => {
          let selectedCount = 0;
          const seen = new Set();

          (Array.isArray(matches) ? matches : []).forEach((match) => {
            const matchId = String(getMatchId(match) || '').trim();
            const matchName = String(getMatchName(match) || '').trim();
            const key = matchId ? `id:${matchId}` : (matchName ? `name:${normalizeName(matchName)}` : '');
            if (!key || seen.has(key)) {
              return;
            }
            seen.add(key);
            ...
            let button = null;
            if (matchId) {
              button = ingredientButtons.find((item) => String(item.dataset.ingId || '') === matchId);
            }
            if (!button && matchName) {
              button = findBestButtonMatch(matchName);
            }
            if (!button) {
              return;
            }
        ...
        const analyzePhoto = async () => {
          ...
          try {
            ...
            const ingredientNames = data.ingredients
              ...
              .filter(Boolean);
            const matchedIngredients = Array.isArray(data.matches) ? data.matches : [];
            const unmatchedIngredients = Array.isArray(data.unmatched) ? data.unmatched : [];
            const hasMatches = matchedIngredients.length > 0;

            if (ingredientNames.length === 0 && !hasMatches) {
              setAiStatus('No ingredients detected. Try another photo.');
              return;
            }

            if (hasMatches) {
              renderAiResults(matchedIngredients, unmatchedIngredients);
              const selectedCount = applyDetectedMatches(matchedIngredients);
              const detectedCount = ingredientNames.length || matchedIngredients.length;
              if (selectedCount > 0) {
                const unmatchedCount = unmatchedIngredients.length;
                const unmatchedNote = unmatchedCount > 0 ? `, ${unmatchedCount} not found in your list` : '';
                setAiStatus(`Detected ${detectedCount} ingredient(s). Matched ${matchedIngredients.length} in your list, added ${selectedCount}${unmatchedNote}.`);
              } else {
                setAiStatus(`Detected ${detectedCount} ingredient(s). Matched ${matchedIngredients.length} in your list, but none were added.`);
              }
            } else {
